...
def lauf(*, source: Path, outbox: Path, state: Path, tag: date | None = None,
    ...
    `alle_spiele`: ueber ALLE Spiele hinweg suchen statt aus einem Ordner (manuelle Themen-Reels).
    `min_dauer`: Mindestlaenge in Sekunden (Default 15). Kuerzere Reels werden NICHT eingereicht.
    """
    ...
    if alle_spiele:
        gewaehlt, index_allow, label = None, set(verfuegbar), "alle Spiele"
    else:
        ...
        index_allow, label = {gewaehlt}, gewaehlt
    ...
    if gewaehlt:                                       # Spiel-Rotation nur bei Einzelspiel protokollieren
...
def main(argv=None) -> int:
    ...
    p.add_argument("--alle-spiele", action="store_true",
                   help="Ueber ALLE Spiele hinweg suchen (manueller Themen-Reel) statt aus einem Ordner.")
