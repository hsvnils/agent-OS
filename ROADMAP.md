...
| 24 | **Skill-/Charta-Standard + gepruefter Skill-Import** (Agent-Skills-Format + Erfolgsmetriken je Charta; Import via Phase-22-Gate) | 🟡 **(c)+(a) fertig 2026-07-03** — (c) Skill-Security-Gate `core/skill_gate.py` (AST/Injection/Shell-Scan, Verdikt bestanden\|pruefen\|abgelehnt + Score, SARIF, Tool `skill_pruefen`, fuehrt nie aus); (a) Format-Standard `governance/skill-standard.md` + Validator `core/skill_format.py` (skill-card-Frontmatter, dependency-frei). **Offen (b):** Erfolgsmetriken je Charta = **CEO-Tor** (Diff-Vorlage HoA->CEO, AGENTS.md 3.3) |
