...
| 22 | **CISO-Agent ausbauen** (Static-Security-Scan nach SkillSpector-Muster) | ✅ **Kern fertig 2026-07-03** (AST-Code-Scan + Risiko-Score 0-100; OSV.dev-Check der Dockerfile-Pins; unser Code + Pins = 0 Funde). **Inkr.4+5 fertig 2026-07-03:** erweiterte Injection-Muster (`input_guard.py`: jailbreak-persona, neue-anweisung, chat-template-token, kodierte-nutzlast) + **SARIF-2.1.0-Export** (`nach_sarif`/`SecurityAgent.sarif`; Tool-Flag `sicherheits_audit(sarif=true)`). **Optionaler Backlog Rest:** Taint-Tracking |
