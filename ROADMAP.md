...
| 22 | **CISO-Agent ausbauen** (Static-Security-Scan nach SkillSpector-Muster) | ✅ **Kern fertig 2026-07-03** (AST-Code-Scan + Risiko-Score 0-100; OSV.dev-Check der Dockerfile-Pins; unser Code + Pins = 0 Funde). **Inkr.4+5 fertig 2026-07-03:** erweiterte Injection-Muster (`input_guard.py`: jailbreak-persona, neue-anweisung, chat-template-token, kodierte-nutzlast) + **SARIF-2.1.0-Export** (`nach_sarif`/`SecurityAgent.sarif`; Tool-Flag `sicherheits_audit(sarif=true)`). **Inkr.3 fertig 2026-07-03:** Taint-Tracking (`analysiere_taint`/`_check_taint`) -- meldet Fluesse extern/Parameter -> Code-Sink (eval/exec/os.system/shell), praeziser als der grobe AST-Scan; unser Code = 0 Funde. **Phase 22 = vollstaendig.** |
...
| 24 | **Skill-/Charta-Standard + gepruefter Skill-Import** (Agent-Skills-Format + Erfolgsmetriken je Charta; Import via Phase-22-Gate) | ✅ **fertig 2026-07-03** — (c) Skill-Security-Gate `core/skill_gate.py` (AST/Injection/Shell-Scan, Verdikt bestanden\|pruefen\|abgelehnt + Score, SARIF, Tool `skill_pruefen`, fuehrt nie aus); (a) Format-Standard `governance/skill-standard.md` + Validator `core/skill_format.py` (skill-card-Frontmatter, dependency-frei); (b) **Erfolgsmetriken & Deliverables** in `_TEMPLATE.md` + allen 18 Charten (CEO-freigegeben mit Diff, AGENTS.md 3.3) |
| 25 | **Execution-Sandbox-Policy** (deklarativ; Blaupause fuer Phase 17, aus OpenShell/NemoClaw) | 🟡 **Blaupause fertig 2026-07-03** — Policy-Engine `core/sandbox_policy.py` (Datei/Netz = default-deny allow-list, Prozess = deny-list, Traversal-Schutz, creds_env_only), Default `governance/sandbox-policy.json`, Doku `governance/sandbox-policy.md`, Tool `sandbox_check`. **Live-Enforcement** greift erst mit Phase 17 (daran gekoppelt) |
| 26 | **Gedaechtnis: Vektor-Recall + Trajektorien-Lernen** (optional, aus MemPalace/ruflo) | ✅ **umgesetzt 2026-07-03** (dependency-frei, local-first): (a) **BM25-Ranking** `core/ranking.py` -> `Brain.suchen` aufgewertet (Term-Frequenz + IDF + Doc-Laenge statt Set-Overlap); (b) **Trajektorien-Store** `core/trajektorien.py` ("was hat funktioniert", BM25-Recall) + Tools `erfahrung_merken`/`erfahrung_abrufen`. **Bewusst KEINE** neuronalen Embeddings (Widerspruch zu token-frugal + keine Ausleitung) |
...
| Roadmap | Thema | Art |
|---|---|---|
| `BETRIEBSLUECKEN_ROADMAP.md` | Betriebsluecken schliessen (BF-01/03/04/08) | geplant (2026-09-25) |
...
- **Phase 23 — Haertung externer Eingaben (Prompt-Injection-/PII-Filter) [HOCH-MITTEL]:**
  ...
  - **Umsetzung (Inkrement 2, 2026-07-03) -- KERN FERTIG:** Filter zusaetzlich verankert in (a) **Web-Recherche**
    (`governance/web_research._finish` -- zentraler Choke-Point, in dem ohnehin Secrets redigiert werden:
    kombinierter externer Text wird auf Injection geprueft, neues Feld `RechercheErgebnis.sicherheit` + Marker
    an der Zusammenfassung) und (b) **Instagram-DMs/kuenftige Kanaele** (`core/crm.verarbeite_eingang` --
    zentral fuer alle DM-Eingaenge; Klassifikation laeuft auf dem Originaltext, gespeichert wird der markierte).
