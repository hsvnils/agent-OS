...
| **Gemini-Video-Modell fuer Reel-Content-Tagging** | 2026-07-07 | **gemini-2.5-flash gewaehlt** (CEO). Genauigkeits-Sweetspot fuer die Tor/Jubel/Choreo-Klassifikation; flash-lite ($0,10/1M) zu grob, pro/3.5-flash Overkill fuer 8-Wort-Vokabular. Umschaltbar per `CUTTER_TAG_MODEL` (keine Code-Aenderung). Native Video-Ingestion -> Gemini statt Claude/GPT (kein Frame-Sampling). Kosten ~0,3-0,9 Cent/Clip, ganze Bibliothek (~1854) einmalig grob **~6-18 EUR**, gecacht. Spaeter: lokales Vision-Modell (0 EUR). | Reel-Pipeline `cutter/reel_tag.py` |
...
| **`qwen3:30b-a3b` auf dem MACO470 fuer Chat/Jobs** (Messung unter Last) | 2026-09-25 | **VERWORFEN** (fuer diese Hardware) | Mit Grafik ~32 GB Bedarf bei 29,6 GB RAM (~12,7 GB Grundlast) -> Dauer-Auslagerung; nur CPU 34,6 GB, 17 Tok/s Prompt. Revidiert die Einschaetzung „20-30B machbar" vom 2026-07-14 fuer den Parallelbetrieb mit Windows-Desktop. Naechster Kandidat: ~14B |
| **`qwen3:14b` auf dem MACO470** (Test mit LUNAs Werkzeugliste) | 2026-09-26 | **ZURUECKGESTELLT** | Passt in den Grafikspeicher (11,9 GB), aber 5/10 Antworten Zeichensalat/Wiederholungen (BF-23) in der aktuellen Server-Konfiguration; Ursache offen |
